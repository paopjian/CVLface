...
if __name__ == '__main__':
    ...
    # Bind each torchrun worker before constructing any CUDA-backed object.
    # Without this, every rank can initialize a context on physical GPU 0
    # before Fabric moves the model, wasting about 384 MiB per rank there.
    ...
    # evaluation callbacks. External evaluation builds these only in its child process.
    evaluators = []
    ...
    for epoch in range(train_pipeline.start_epoch, cfg.optims.num_epoch):
        ...
        if cfg.evaluations.eval_every_n_epochs > 0 and model.has_trainable_params():
            ...
            if should_evaluate and cfg.trainers.external_eval:
                # Drop epoch-local training tensors before the child loads a second model copy.
                batch = None
                loss = None
            ...
            if fabric.local_rank == 0:
                if all_result:
                    ...
                    fabric.log_dict(summary_dict, step=step)
                    # MLflow 记录评估指标
