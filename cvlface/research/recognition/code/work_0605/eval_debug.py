...
if __name__ == '__main__':
    ...
    parser.add_argument('--eval_config_name', type=str, default='work2')
    parser.add_argument('--pipeline_name', type=str, default='default')
    parser.add_argument('--ckpt_dir', type=str, default="/root/zhaokj/CVLface/cvlface/pretrained_models/recognition/s3_full_01_26")
    ...
    parser.add_argument('--save_path', type=str, default='')
    parser.add_argument('--image_dir', type=str, default='')
    parser.add_argument('--save_embedding', type=str, default='0')
    ...
    for name, info in eval_config.per_epoch_evaluations.items():
        ...
        eval_type =='custom_verification3'
        ...
        if args.save_embedding == '1':
            ...
            if fabric.local_rank == 0:
                with open(args.save_path, 'wb') as f:
                    pickle.dump({'collection': collection, 'collection_flip': collection_flip}, f)
            continue
            
        if evaluator.name == 'work':
            result = evaluator.evaluate(eval_pipeline, epoch=0, step=0, n_images_seen=0, save_image_path = args.save_path, save_pkl=args.save_path, image_dir=args.image_dir)
        else:
