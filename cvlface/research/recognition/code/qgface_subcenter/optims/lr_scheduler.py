...
def make_scheduler(cfg, opt, base_lr=None, warmup_steps=None):
    base_lr = cfg.optims.lr if base_lr is None else base_lr
    warmup_steps = cfg.trainers.warmup_step if warmup_steps is None else warmup_steps
    ...
    if cfg.optims.scheduler == 'poly_2':
        ...
        lr_scheduler = CosineLRScheduler(opt,
                                         t_initial=cfg.trainers.total_step - warmup_steps,
...
class StepScheduler(_LRScheduler):
    def __init__(self, optimizer, base_lr, max_steps, warmup_steps, warmup_lr_init=0.0001, lr_milestones=[], lr_lambda=0.1, last_epoch=-1):
        ...
        super(StepScheduler, self).__init__(optimizer=optimizer, last_epoch=-1)
