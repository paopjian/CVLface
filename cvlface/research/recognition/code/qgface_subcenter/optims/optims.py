...
def _make_optimizer(params, cfg, lr):
    if cfg.optims.optimizer == "sgd":
        return torch.optim.SGD(
            params=params,
            lr=lr,
            momentum=cfg.optims.momentum,
            weight_decay=cfg.optims.weight_decay,
        )
    if cfg.optims.optimizer == "adamw":
        return torch.optim.AdamW(
            params=params,
            lr=lr,
            weight_decay=cfg.optims.weight_decay,
        )
    raise ValueError(f"Unsupported optimizer: {cfg.optims.optimizer}")


def _model_param_groups(cfg, model):
    no_weight_decay_value = 0.0 if cfg.optims.filter_bias_and_bn else cfg.optims.weight_decay
    no_weight_decay_list = []
    for name, _ in model.named_parameters():
        if ('emb' in name and 'patch_embed' not in name) or ('token' in name):
            no_weight_decay_list.append(name)
    return param_groups_weight_decay(
...
def make_qgface_optimizers(cfg, model, classifier):
    """Create independent optimizers so PFC gradients cannot dominate backbone clipping."""
    model_optimizer = None
    classifier_optimizer = None

    if model.has_trainable_params():
        model_optimizer = _make_optimizer(
            _model_param_groups(cfg, model),
            cfg,
            lr=cfg.optims.lr,
        )
    ...
    if classifier is not None and classifier.has_trainable_params():
        ...
        classifier_weight_decay = getattr(
            cfg.optims, 'classifier_weight_decay', cfg.optims.weight_decay
        )
        classifier_params = [{
            "params": [p for p in classifier.parameters() if p.requires_grad],
            "weight_decay": classifier_weight_decay,
        }]
        classifier_optimizer = _make_optimizer(
            classifier_params,
            cfg,
            lr=classifier_lr,
        )

    if model_optimizer is None and classifier_optimizer is None:
        raise ValueError("QGFace requires at least one trainable model or classifier parameter")
...
def make_optimizer(cfg, model, classifier, aligner):
    ...
    return _make_optimizer(params, cfg, lr=cfg.optims.lr)
