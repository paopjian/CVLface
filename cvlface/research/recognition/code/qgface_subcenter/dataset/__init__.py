...
from .contrastive_view_dataset import ContrastiveViewDataset
from .qgface_view_transform import QGFaceViewTransform
...
def get_train_dataset(dataset_cfg, train_transform, aug_cfg, local_rank=0):
    ...
    # Image Folder (with parquet-cached index for fast startup)
    else:
        from .cached_image_folder import CachedImageFolderDataset
        train_set = CachedImageFolderDataset(
            root_dir=root_dir, transform=train_transform, local_rank=local_rank,
        )
        label_mapping = None

    train_set.color_space = dataset_cfg.color_space
    if getattr(aug_cfg, 'contrastive_views', False):
        view_transform = None
        if hasattr(aug_cfg, 'qgface_view_params'):
            view_transform = QGFaceViewTransform(**dict(aug_cfg.qgface_view_params))
        train_set = ContrastiveViewDataset(train_set, view_transform=view_transform)
