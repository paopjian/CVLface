...
class AdaFaceLoss(torch.nn.Module):
    ...
    def forward(self, logits, labels, norms, batch_mean, batch_std):
        ...
        positive_margin_scaler = margin_scaler[index_positive]
        ...
        with torch.no_grad():
            ...
            margin_final_logit = target_logit + (self.m * positive_margin_scaler * -1)
            margin_final_logit.cos_()
            # g_additive
            margin_final_logit = margin_final_logit - (
                self.m + (self.m * positive_margin_scaler)
            )
        ...
        if self.quality_scale != 0:
            quality_scaler = torch.sigmoid(
                (margin_scaler + 2 * (0.5 - self.quality_scale)) * self.s
            )
            logits = logits * quality_scaler.view(-1, 1).to(logits.dtype)
