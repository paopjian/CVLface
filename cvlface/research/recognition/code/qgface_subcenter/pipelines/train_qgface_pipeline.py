    ...
    def __call__(self, batch):
        ...
        if self.contrast_weight > 0:
            ...
            with torch.no_grad():
                ...
                subcenter_ids = self._classifier_method(
                    "route_subcenters", all_labels, all_original
                )
                queue_batch_subcenter_ids = torch.cat(
                    [subcenter_ids, subcenter_ids], dim=0
                )
                ...
                _, queued_labels, queued_subcenter_ids = self.qgface_loss.get_queue()
                current_proxies = self._classifier_method(
                    "get_class_proxies", queued_labels, queued_subcenter_ids
                )
                queue_embeddings, queued_labels, _ = self.qgface_loss.get_queue(
                    current_proxies
                )
