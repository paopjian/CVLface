...
class TinyClassifier(torch.nn.Module):
    ...
    def forward(self, embeddings, labels):
        ...
        logits = torch.nn.functional.linear(
            ...
            torch.nn.functional.normalize(self.weight, dim=1),
        )
...
def test_loss_and_queue():
    ...
    loss_fn.enqueue(key, labels, proxies)
    loss_fn.enqueue(key + 1, labels, proxies)
    queued_embeddings, queued_labels = loss_fn.get_queue()
    assert queued_embeddings.shape == (5, 8)
    assert queued_labels.shape == (5,)

    current_proxies = loss_fn.queue_proxies[:5] + 0.1
    updated_embeddings, _ = loss_fn.get_queue(current_proxies)
