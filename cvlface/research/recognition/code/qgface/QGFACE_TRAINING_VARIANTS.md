...
| 6 | IR-101 | 方案 5 epoch 11 | PFC 40% | 分类器重对齐 5 epoch，然后冻结分类器训练 backbone 15 epoch |
...
方案 2-6 使用 `sample_rate=0.40`：全部正类始终参与，每一步采样 40% 负类。
...
方案 6 默认自动选择最新方案 5 运行的 epoch 11 checkpoint，同时加载其中的 backbone
和 8 个分类器分片。第一阶段冻结 backbone，以 LR `0.006` 对分类器重对齐 5 epoch；
第二阶段加载重对齐后的分类器并冻结，以 LR `0.0008`、2 epoch warmup 和 cosine 调度
训练 backbone 15 epoch。可以显式指定输入或跳过已完成的分类器阶段：

```bash
MODE5_EPOCH11_CKPT=/data1/dataset_0605/train_output/<mode5-run>/checkpoints_every_epoch/<epoch11> \
  bash scripts/examples/run_qgface_variants_0605.sh 6

STAGE6_CLASSIFIER_CKPT=/data1/dataset_0605/train_output/<mode6-classifier-run>/checkpoints_every_epoch/<epoch4> \
  bash scripts/examples/run_qgface_variants_0605.sh 6
```

方案 6 使用固定的分阶段学习率，若环境中残留 `LR` 或 `CLASSIFIER_LR` 会直接退出，
避免方案 5 实验中全局 `LR=0.1` 意外覆盖 backbone 微调学习率的问题。方案 6 同时
固定每卡 batch size 为 `128`（全局 batch size `1024`）；外部 `BATCH_SIZE` 不会覆盖它。
...
方案 5 固定为 5+12 epoch，方案 6 固定为 5+15 epoch，均不接受 `NUM_EPOCH` 覆盖。
