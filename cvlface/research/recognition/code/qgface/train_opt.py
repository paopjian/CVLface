...
from optims.optims import make_optimizer, make_qgface_optimizers
...
if __name__ == '__main__':
    ...
    strategy = (
        DDPStrategy(timeout=datetime.timedelta(minutes=nccl_timeout_min))
    ...
    if fabric.local_rank == 0:
        ...
        mlflow_params = {
            ...
            "model_lr": cfg.optims.lr,
            "classifier_lr": getattr(cfg.optims, 'classifier_lr', cfg.optims.lr),
    ...
    # channels_last 加速卷积；QGFace 两路图像由同一个编码器统一处理。
    ...
    is_qgface_pipeline = cfg.pipelines.name == 'TrainQGFacePipeline'
    optimizer = None
    lr_scheduler = None
    model_optimizer = None
    classifier_optimizer = None
    model_lr_scheduler = None
    classifier_lr_scheduler = None
    dummy_model = None

    if is_qgface_pipeline:
        if aligner.has_trainable_params():
            raise ValueError("TrainQGFacePipeline does not support a trainable aligner")
        # QGFace 的超大分类器必须与 backbone 使用独立优化器和裁剪预算。
    ...
    if is_qgface_pipeline and classifier_optimizer is not None:
    ...
    if classifier is not None and getattr(classifier, 'apply_ddp', False):
    ...
    for epoch in range(train_pipeline.start_epoch, cfg.optims.num_epoch):
        ...
        for batch_idx, batch in enumerate(dataloader):
                    ...
                    if is_qgface_pipeline:
                        if model_optimizer is not None:
                            fabric.clip_gradients(
                                model,
                                model_optimizer,
                                max_norm=cfg.optims.max_grad_norm,
                            )
                        ...
                        if classifier_optimizer is not None:
                            fabric.clip_gradients(
                                ...
                                max_norm=getattr(
                                    ...
                                    cfg.optims.max_grad_norm,
                                ),
                            )
                            classifier_optimizer.step()
                            classifier_optimizer.zero_grad(set_to_none=True)
                    else:
                        clip_module = model if model.has_trainable_params() else dummy_model
                        fabric.clip_gradients(
                            clip_module,
                            optimizer,
                            max_norm=cfg.optims.max_grad_norm,
            ...
            if is_qgface_pipeline:
                if model_lr_scheduler is not None:
                    scheduler_step(model_lr_scheduler, step)
                if classifier_lr_scheduler is not None:
                    scheduler_step(classifier_lr_scheduler, step)
                model_lr = (
                    get_last_lr(model_optimizer) if model_optimizer is not None else 0.0
                )
                classifier_lr = (
                    get_last_lr(classifier_optimizer)
                    if classifier_optimizer is not None
                    else 0.0
                )
                last_lr = model_lr if model_optimizer is not None else classifier_lr
            else:
                scheduler_step(lr_scheduler, step)
                last_lr = get_last_lr(optimizer)
                model_lr = last_lr if model.has_trainable_params() else 0.0
                classifier_lr = last_lr if classifier is not None else 0.0
            ...
            if batch_idx % 200 == 0:
                ...
                log_dict['train/model_lr'] = model_lr
                log_dict['train/classifier_lr'] = classifier_lr
                ...
                if hasattr(train_pipeline, 'get_log_dict'):
                    log_dict.update(train_pipeline.get_log_dict())
        ...
        # 每个epoch保存模型
        fabric.barrier()
        model_save_dir = getattr(cfg.dataset, 'model_save_dir', '')
        if model_save_dir:
