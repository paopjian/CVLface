...
class LearningRateManager:
    def __init__(self, patience=5, threshold=0.01, factor=0.5, min_lr=1e-10):
        self.patience = patience  # 连续多少个epoch没有改进后降低lr
        self.threshold = threshold  # 改进的最小阈值
        self.factor = factor  # 学习率降低的因子
        self.min_lr = min_lr  # 最小学习率
        
        self.best_loss = float('inf')
        self.loss_history = []  # 存储每个epoch的平均损失
    ...
    def update_loss_history(self, loss):
        ...
        # 如果连续patience个epoch没有改进，返回True表示需要降低学习率
        return self.no_improvement_count >= self.patience
    
    def should_reduce_lr_grad(self, grad_norm, update_ratio):
        """根据梯度和更新比率检查是否需要降低学习率"""
        return grad_norm > 50 or update_ratio > 1e-2
    
    def reduce_learning_rate(self, optimizer):
        """降低学习率"""
...
if __name__ == '__main__':    
    ...
    # 初始化学习率管理器和早停监视器
    lr_manager = LearningRateManager(patience=5, threshold=0.01, factor=0.5)
    ...
    for epoch in range(train_pipeline.start_epoch, cfg.optims.num_epoch):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            # 如果梯度范数或更新比率过大，减少学习率
            if lr_manager.should_reduce_lr_grad(grad_norm_backbone, update_ratio_backbone):
                reduced = lr_manager.reduce_learning_rate(optimizer)
                if reduced:
                    print(f"Epoch {epoch} | Reduced learning rate due to large gradient norm ({grad_norm_backbone:.4f}) or update ratio ({update_ratio_backbone:.4f})")
            # 记录loss
        ...
        # 根据损失历史更新学习率
        if lr_manager.update_loss_history(avg_epoch_loss):
            reduced = lr_manager.reduce_learning_rate(optimizer)
            if reduced:
                print(f"Epoch {epoch} | Reduced learning rate due to no significant loss improvement for {lr_manager.patience} epochs")
        # 每个epoch保存模型
        save_dir = os.path.join(cfg.trainers.output_dir, 'checkpoints_every_epoch', f'epoch:{epoch}_step:{step}')
