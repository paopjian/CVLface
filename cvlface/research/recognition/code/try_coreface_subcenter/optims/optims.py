...
def _component_lr(optim_cfg, name):
    component_lr = getattr(optim_cfg, name, None)
    return optim_cfg.lr if component_lr is None else component_lr
...
def make_optimizer(cfg, model, classifier, aligner):
    ...
    model_lr = _component_lr(cfg.optims, 'model_lr')
    classifier_lr = _component_lr(cfg.optims, 'classifier_lr')
    aligner_lr = _component_lr(cfg.optims, 'aligner_lr')
    ...
    if model.has_trainable_params():
        ...
        for param_group in model_param_groups:
            param_group['lr'] = model_lr
            param_group['group_name'] = 'model'
    ...
    print(f"Model learning rate: {model_lr}")
    if classifier is not None and classifier.has_trainable_params():
