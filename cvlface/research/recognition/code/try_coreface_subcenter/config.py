...
def prepare_output_dir(cfg, root):
    ...
    requested_output_dir = str(getattr(cfg.trainers, 'output_dir', '') or '')
    if requested_output_dir:
        output_dir = os.path.abspath(os.path.expanduser(requested_output_dir))
        if not cfg.pipelines.resume:
            raise ValueError(
                'trainers.output_dir may only be supplied together with trainers.resume'
            )
        if not os.path.isdir(output_dir):
            raise FileNotFoundError(f'resume output directory not found: {output_dir}')
        print('resume output_dir ', output_dir)
        return output_dir
