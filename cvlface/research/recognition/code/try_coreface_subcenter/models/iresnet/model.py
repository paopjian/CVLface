...
class Backbone(Module):
    ...
    def __init__(self, input_size, num_layers, mode='ir', flip=False, output_dim=512,
        ...
        if input_size[0] == 112:
            ...
            self.output_layer = Sequential(
                BatchNorm2d(output_channel), Dropout(dropout), Flatten(),
    ...
    def set_dropout(self, probability):
        """Update the embedding dropout used by both normal and CoreFace paths."""
        if not 0 <= probability < 1:
            raise ValueError(f'dropout must be in [0, 1), got {probability}')
        self.output_layer[1].p = probability

    def forward_double(self, x, dropout=None):
        """Return two independently dropped embeddings from one shared forward pass.

        CoreFace applies dropout twice after the shared convolutional trunk.  The
        explicit ``training=True`` keeps this augmentation active for the
        partial-backbone fine-tuning stages, where the rest of the model remains
        in eval mode to preserve frozen BatchNorm statistics.
        """
        if self.flip:
            x = x.flip(1)
        x = self.input_layer(x)
...
def IR_18(input_size, output_dim=512, dropout=0.4):
    model = Backbone(input_size, 18, 'ir', output_dim=output_dim, dropout=dropout)

    return model


def IR_34(input_size, output_dim=512, dropout=0.4):
    model = Backbone(input_size, 34, 'ir', output_dim=output_dim, dropout=dropout)

    return model


def IR_50(input_size, output_dim=512, dropout=0.4):
    model = Backbone(input_size, 50, 'ir', output_dim=output_dim, dropout=dropout)

    return model


def IR_101(input_size, output_dim=512, dropout=0.4):
    model = Backbone(input_size, 100, 'ir', output_dim=output_dim, dropout=dropout)

    return model


def IR_101_FLIP(input_size, output_dim=512, dropout=0.4):
    model = Backbone(input_size, 100, 'ir', flip=True, output_dim=output_dim, dropout=dropout)

    return model



def IR_152(input_size, output_dim=512, dropout=0.4):
    model = Backbone(input_size, 152, 'ir', output_dim=output_dim, dropout=dropout)

    return model


def IR_200(input_size, output_dim=512, dropout=0.4):
    model = Backbone(input_size, 200, 'ir', output_dim=output_dim, dropout=dropout)
