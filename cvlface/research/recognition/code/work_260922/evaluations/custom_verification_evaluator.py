...
class CustomVerificationEvaluator(BaseEvaluator):
    def __init__(self, name, data_path, transform, fabric, batch_size, num_workers,
                 type=None, decode='cv2'):
        super().__init__(name, fabric, batch_size)
        self.name = name
        self.batch_size = batch_size
        self.data_path = data_path
        # decode: 'cv2' (默认, 原 DataLoader 路径) / 'auto'|'nvjpeg' (GPU 批量解码,
        # 失败自动降级 cv2; 见 evaluations/nvjpeg_pipeline.py)
        self.decode = decode
        self._nvjpeg_failed = False
        self._nvjpeg_probed = False
        self._nvjpeg_probe_err = None
        self._nv_flip_cache = None
    ...
    def extract(self, pipeline, flip_images=False):
        # nvjpeg GPU 批量解码路径 (decode != cv2)。
        # 降级必须全体 rank 一致: 先本地探测 (构造+首批解码, 无集合通信), 再
        # all_reduce 表决 —— 任一 rank 失败全体降级。否则部分 rank 走 nvjpeg、
        # 部分走 DataLoader, gather 的 barrier 错位会死锁到 NCCL timeout。
        if self.decode != 'cv2' and not self._nvjpeg_failed:
            if not self._nvjpeg_probed:
                try:
                    self._probe_nvjpeg()
                    self._nvjpeg_probe_err = None
                except Exception as e:  # noqa: BLE001
                    self._nvjpeg_probe_err = e
                self._nvjpeg_probed = True
            flag = torch.tensor(
                [0.0 if self._nvjpeg_probe_err is None else 1.0],
                device=self.fabric.device)
            self.fabric.all_reduce(flag)
            if flag.item() > 0:
                if self._nvjpeg_probe_err is not None:
                    print(f'[nvjpeg] {self.name} 探测失败 '
                          f'({type(self._nvjpeg_probe_err).__name__}: '
                          f'{self._nvjpeg_probe_err}), 全体 rank 降级 cv2 DataLoader')
                self._nvjpeg_failed = True
                self._nv_flip_cache = None
            elif flip_images and self._nv_flip_cache is not None:
                cached = self._nv_flip_cache
                self._nv_flip_cache = None
                # 非 rank0 时 cached 是哨兵 'done' (gather 在非 rank0 返回 None,
                # 其结果本就不被 evaluate 使用; 不能存 None, 否则 flip 调用会
                # 在非 rank0 重跑整遍解码+推理)
                return None if cached == 'done' else cached
            else:
                # 循环内异常 (探测通过后属罕见 GPU 故障) 直接抛出:
                # 半途做集合通信同样会 barrier 错位, 宁可显式失败
                coll_n, coll_f = self._extract_nvjpeg(pipeline)
                if flip_images:
                    return coll_f
                self._nv_flip_cache = coll_f if coll_f is not None else 'done'
                return coll_n
    ...
    def _probe_nvjpeg(self):
        """本地探测 (无集合通信): feeder 构造 (依赖/格式/init) + 首批真实解码。"""
        from evaluations.nvjpeg_pipeline import NvjpegFeeder, detect_kind
        kind = detect_kind(self.data_path)
        if kind == 'hf':
            raise RuntimeError('hf 数据源暂不走 nvjpeg')
        feeder = NvjpegFeeder(kind, self.data_path, self.fabric.local_rank,
                              self.fabric.world_size, self.batch_size,
                              self.fabric.local_rank)
        if len(feeder) == 0:
            raise RuntimeError(
                f'分片为空 (数据 {feeder.n_total} × world {self.fabric.world_size})')
        next(iter(feeder.batches(torch.cuda.current_stream())))
        torch.cuda.synchronize()

    def _extract_nvjpeg(self, pipeline):
        """nvjpeg GPU 批量解码提特征 (rec/folder; 基于 portable_eval 管线)。

        一次解码同时算 normal/flip 两份特征, 返回 (normal collection, flip
        collection); evaluate() 先后两次 extract 分别取用, 免二次解码。
        分片补齐到 world 整除 (重复开头索引), index 为全局行号, 与原路径
        gather (stack+去重) 语义一致。
        """
        from evaluations.nvjpeg_pipeline import NvjpegFeeder, detect_kind
        kind = detect_kind(self.data_path)
        rank = self.fabric.local_rank
        world = self.fabric.world_size
        feeder = NvjpegFeeder(kind, self.data_path, rank, world,
                              self.batch_size, rank)

        feats_n, feats_f, labels_l, index_l = [], [], [], []
        stream = torch.cuda.current_stream()  # 解码/归一化/推理同流, 顺序天然正确
        for b in tqdm(feeder.batches(stream), total=len(feeder),
                      desc=f'nvjpeg {self.name}', disable=rank != 0):
            x = b.x[:b.n_valid]
            with torch.no_grad():
                feats_n.append(pipeline(x).float().cpu())
                feats_f.append(pipeline(torch.flip(x, dims=[3])).float().cpu())
            labels_l.append(torch.from_numpy(np.asarray(b.labels, dtype=np.int64)))
            index_l.append(torch.from_numpy(b.index))

        coll_f = {'labels': torch.cat(labels_l, dim=0),
                  'features': torch.cat(feats_f, dim=0),
                  'index': torch.cat(index_l, dim=0)}
        coll_n = {'labels': torch.cat(labels_l, dim=0),
                  'features': torch.cat(feats_n, dim=0),
                  'index': torch.cat(index_l, dim=0)}
        gathered_f = self.gather_collection(method='cpu', per_gpu_collection=coll_f)
        torch.cuda.empty_cache()
        gathered_n = self.gather_collection(method='cpu', per_gpu_collection=coll_n)
        return gathered_n, gathered_f
