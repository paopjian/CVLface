...
def apply_peft_to_model(peft_config, model):
    ...
    if peft_config.name == 'lora':
        ...
        # The legacy iResNet implementation exposes 49 residual blocks as
        # ``body.0`` ... ``body.48``.  InsightFace iResNet-100 represents the
        # same blocks as layer1[0:3], layer2[0:13], layer3[0:30], layer4[0:3].
        # InsightFace iResNet-200 represents 98 blocks as
        # layer1[0:6], layer2[0:26], layer3[0:60], layer4[0:6].
        # Keep ``body.K`` as the public PEFT selector while supporting both
        # parameter naming schemes.

        # IR-100 mapping (49 blocks)
        insightface_blocks_ir100 = (
            [f'net.layer1.{i}' for i in range(3)]
            + [f'net.layer2.{i}' for i in range(13)]
            + [f'net.layer3.{i}' for i in range(30)]
            + [f'net.layer4.{i}' for i in range(3)]
        )
        for k in range(49):
            target_modules_mapping[f'body.{k}'] = (
                [f'body.{i}' for i in range(k, 49)]
                + ['output_layer']
                + insightface_blocks_ir100[k:]
                + ['net.bn2', 'net.fc', 'net.features']
            )

        # IR-200 mapping (98 blocks)
        insightface_blocks_ir200 = (
            [f'net.layer1.{i}' for i in range(6)]
            + [f'net.layer2.{i}' for i in range(26)]
            + [f'net.layer3.{i}' for i in range(60)]
            + [f'net.layer4.{i}' for i in range(6)]
        )
        for k in range(98):
            target_modules_mapping[f'body.{k}'] = (
                [f'body.{i}' for i in range(k, 98)]
                + ['output_layer']
                + insightface_blocks_ir200[k:]
                + ['net.bn2', 'net.fc', 'net.features']
            )

        # Direct layer access for more intuitive configuration
        # IR-100: layer1[0:3], layer2[0:13], layer3[0:30], layer4[0:3]
        for layer_idx in range(1, 5):
            layer_sizes = {1: 3, 2: 13, 3: 30, 4: 3}  # IR-100
            for block_idx in range(layer_sizes[layer_idx]):
                target_key = f'layer{layer_idx}.{block_idx}'
                # Convert to body index for IR-100
                body_start = sum(layer_sizes[i] for i in range(1, layer_idx)) + block_idx
                target_modules_mapping[target_key] = target_modules_mapping[f'body.{body_start}']

        # IR-200: layer1[0:6], layer2[0:26], layer3[0:60], layer4[0:6]
        for layer_idx in range(1, 5):
            layer_sizes_ir200 = {1: 6, 2: 26, 3: 60, 4: 6}  # IR-200
            for block_idx in range(layer_sizes_ir200[layer_idx]):
                target_key = f'layer{layer_idx}.{block_idx}'
                # Find all modules from this layer.block onwards
                modules_to_unfreeze = []
                # Add all subsequent blocks in current layer
                for j in range(block_idx, layer_sizes_ir200[layer_idx]):
                    modules_to_unfreeze.append(f'net.layer{layer_idx}.{j}')
                # Add all blocks in subsequent layers
                for next_layer in range(layer_idx + 1, 5):
                    for j in range(layer_sizes_ir200[next_layer]):
                        modules_to_unfreeze.append(f'net.layer{next_layer}.{j}')
                modules_to_unfreeze += ['net.bn2', 'net.fc', 'net.features']
                target_modules_mapping[target_key] = modules_to_unfreeze
        target_modules = target_modules_mapping[peft_config.target_modules]

        for key, param in model.named_parameters():
            is_train = any(key == target or key.startswith(target + '.')
