...
class FCClassifier(BaseClassifier):
    ...
    def __init__(self, classifier, config, rank, world_size):
        ...
        # TopoFR needs the raw input images for the topological loss. margin_softmax
        # lives on the inner FC, so expose the flag here for the training pipeline.
    ...
    def forward(self, local_embeddings, local_labels, input_images=None):
        if self.is_topofr:
