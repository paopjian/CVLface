...
# 低显存版: 共享的 dist_utils 版本对 791509 类的 FC 头做 all_gather 会要 ~13 GB 而 OOM.
from ddp_verify_lowmem import verify_ddp_weights_equal
...
def broadcast_should_stop(fabric, should_stop):
    ...
    fabric.broadcast 底层走 broadcast_object_list，返回的是反序列化后的新对象，
    不会原地修改传入的 tensor。必须使用返回值，否则非 0 rank 读到的仍是自己的
    初始值，导致 rank0 退出而其余 rank 继续训练并卡死在后续 barrier 上。
    """
    stop_tensor = torch.tensor(int(should_stop), device=fabric.device)
    stop_tensor = fabric.broadcast(stop_tensor, src=0)
...
if __name__ == '__main__':
                ...
                if all_result:
                    for monitor in improvement_monitors:
                        if monitor.check(all_result):
                            ...
                            should_stop = True
            should_stop = broadcast_should_stop(fabric, should_stop)

            # save model
            # 必须在 break 之前保存：早停时最后一个 epoch 同样需要落盘完整 checkpoint，
            # 否则下一阶段只能回退到上一个 epoch。
    ...
    # 收尾前对齐所有 rank：rank0 之后要独自 join MLflow 线程并 finish wandb，
    # 其余 rank 若直接冲到进程退出，会卡在 destroy_process_group 的集合通信上
    # 直到 NCCL 超时（默认 2h）被 watchdog 强杀。
    fabric.barrier()
