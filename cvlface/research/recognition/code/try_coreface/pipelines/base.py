...
class BasePipeline():
    ...
    def resume_from_dir(self, resume_dir):
        ...
        pipeline_st = torch.load(pipepline_path, map_location='cpu', weights_only=False)
        ...
        for name in self.module_names_list:
            ...
            if hasattr(mod, 'load_state_dict_from_path'):
                mod_path = pipepline_path.replace('pipeline.pt', f'{name}.pt')
                if not os.path.isfile(mod_path):
                    mod_path = resume_dir
                mod.load_state_dict_from_path(mod_path)
            else:
                mod_st = torch.load(pipepline_path.replace('pipeline.pt', f'{name}.pt'), map_location='cpu', weights_only=False)
    ...
    def save(self, fabric, pipeline, cfg, epoch, step, n_images_seen, is_best=False):
        ...
        # The exact step remains in pipeline.pt; the path is intentionally
        # stable so checkpoint consumers only need to know the epoch.
        save_dir = os.path.join(cfg.trainers.output_dir, 'checkpoints', f'epoch:{epoch}')
